...
重要要求——细节宁多勿少：
1. 菜名：从文字中识别
2. 食材清单：每行一种，格式「食材名 + 用量」。文字里提到的用量必须保留（如 250克、2勺、1块），没提用量的就只写食材名
3. 准备工作：每行一条，写具体——怎么处理、怎么切、腌多久，比如「芦笋削去根部老皮，斜刀切成小段」而不是「芦笋切段」
4. 步骤：按做菜顺序，每条步骤包含：
   - main（主步骤，一句话概括，如「芦笋焯水」）
   - detail（必须详细！直接还原原文的完整做法描述，包括：怎么处理食材、火候大小、时间、状态判断、为什么这样做。比如原文是「把芦笋根部老的地方削皮，斜刀切成小段」就写成「把芦笋根部老的地方削皮，斜刀切成小段」，不要压缩成「芦笋切段」）
   detail 就是原文的详细重述，宁可啰嗦，不要简化，用户会自己删减
5. 掌勺心得：原文中特别提示的注意事项、容易出错的地方，每条写完整
...
返回格式（严格 JSON）：
{
  ...
  "prep": ["五花肉切 2cm 方块，冷水下锅焯水撇浮沫"],
  "steps": [{"main": "炒糖色", "detail": "锅里放冰糖小火慢炒，糖色变琥珀色立即下肉，火大容易炒糊发苦"}],
  "pits": ["糖色容易炒糊，一定小火，变色后动作要快"]
}`;
  }
  // 图片模式
...
重要要求——细节宁多勿少：
1. 菜名：从图片中识别
2. 食材清单：每行一种，格式「食材名 + 用量」。图片里标明的用量必须保留（如 250克、2勺），没标用量的就只写食材名
3. 准备工作：每行一条，写具体——怎么处理、怎么切、腌多久，比如「芦笋削去根部老皮，斜刀切成小段」而不是「芦笋切段」
4. 步骤：按做菜顺序，每条步骤包含：
   - main（主步骤，一句话概括，如「芦笋焯水」）
   - detail（必须详细！把图片里展示的完整做法描述出来，包括：怎么处理食材、火候大小、时间、状态判断、为什么这样做，如「把芦笋根部老的地方削皮，斜刀切成小段」而不是「芦笋切段」）
   detail 就是原图的详细描述，宁可啰嗦，不要简化，用户会自己删减
5. 掌勺心得：图片中特别提示的注意事项、容易出错的地方，每条写完整
